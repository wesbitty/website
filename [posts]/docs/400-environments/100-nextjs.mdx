---
title: Next.js
excerpt: Use the wesjet-nextjs-plugin plugin to tightly integrate Contentlayer into a Next.js project.
---
...
Using Contentlayer in a Next.js project is easiest if you use the [`wesjet-nextjs-plugin`](/docs/reference/wesjet-nextjs-plugin) plugin. Install the plugin:
...
Content live reload should work out-of-the-box with Next.js when using the `wesjet-nextjs-plugin` plugin and importing your content (e.g. `import { allPosts } from 'wesjet/jetpack'`) directly in your page app/components.
...
Preact can be used with a custom Webpack config. See [this GitHub issue](https://github.com/wesbitty/wesbitty/issues/62) for details.
