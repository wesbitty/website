...
excerpt: How Wesjet determines and sets the type for each document.
---

Wesjet resolves the _type_ of each document in this order:
...
Following the example above, if you've specified the `filePathPattern` as `posts/**/*.mdx` and the type as `Post`, you don't want to place a document of type `Post` in a directory outside `posts` or without an `.mdx` file extension. If you do, Wesjet may process the file.
...
Then Wesjet's interpretation would look like this:
