---
title: makeSource
excerpt: makeSource provides Wesjet with the schema and configuration when using local files as the content source.
---

`makeSource` provides Wesjet with the schema and configuration for your application.

## Usage

The code calling `makeSource` should be placed in `wesjet.config.js`.
...
<OptionsTable>
  ...
  <OptionDescription>
    An array of paths that Wesjet should include, with the following conditions:
  ...
  <OptionDescription>
    An array of paths that Wesjet should explicitly exclude, with the following conditions:
