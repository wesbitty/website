...
//   const gitHubPage = await fetch('https://github.com/wesbitty/wesbitty/', { cache: 'no-store' }).then((_) =>
//     _.text(),
//   )
...
const getUsedByCount = async () => {
  // We're scraping the GitHub website here instead of using the API since I don't want to deal with rate limits and API tokens 🤷‍♂️
  const url =
    'https://github.com/wesbitty/wesbitty/network/dependents?dependent_type=REPOSITORY&package_id=UGFja2FnZS0yMTE4Nzg3MDAw'
  const gitHubPage = await fetch(url, { cache: 'no-store' }).then((_) => _.text())
  const $ = cheerio(gitHubPage)

  const usedByCountStr = $(
    'a[href="/wesbitty/wesbitty/network/dependents?dependent_type=REPOSITORY&package_id=UGFja2FnZS0yMTE4Nzg3MDAw"]',
  )
    .text()
    .trim()
