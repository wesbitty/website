...
import { allDocs, allStudios, allPosts, Post } from 'wesjet/jetpack'
...
export const SearchProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const studiosTree = buildStudiosTree(allStudios)
  const actions = useMemo(() => {
    let actions: Action[] = [
      ...
      },
      {
        ...
        section: 'External',
        perform: () => window.open('https://github.com/wesbitty/wesbitty', '_ blank'),
      },
      {
        id: '3-discord',
    ...
    let id = 1
    const mapStudios = (tree: TreeNode[], parent: string) => {
      for (const element of tree) {
        actions.push({
          id: ('1-studio-' + id).toString(),
          name: element.label
            ? `${element.title == 'Studio' ? 'Overview' : element.title} (${element.label})`
            : element.title == 'Studio'
              ? 'Overview'
              : element.title,
        ...
        id++
        if (element.children.length) mapStudios(element.children, parent + parent ? ' / ' : '' + element.title)
      }
    }
    ...
    mapStudios(studiosTree, '')
    mapPosts(allPosts)
    mapDocs(docsTree, 'Docs')
    return actions
  }, [docsTree, studiosTree, router])
